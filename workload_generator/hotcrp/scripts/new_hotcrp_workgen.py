...
def review_payload():
    payload = {
        ...
        'paperSummary': random_string(200),
        'commentsToAuthor': random_string(1400),
        'commentsToPC': random_string(2000)
    }
    return payload
...
#                    p = do_action(sess, "get", edit_url)
                    # edit
                    # we do a [1, max_repeat] random here:
                    real_repeat = random.randint(1, max_repeat)
                    print("  will edit " + str(real_repeat) + " times\n")
                    for i in range(real_repeat):
