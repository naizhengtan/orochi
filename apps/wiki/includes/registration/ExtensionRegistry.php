...
class ExtensionRegistry {
    ...
    // cheng-hack:
    // NOTE: in order to rule out the path in different machines
    $str_key = "";
    foreach ($this->queued as $path => $mtime) {
      $str_key .= substr($path, -20) . "$mtime";
    }
    $hash_key = md5($str_key);

		// See if this queue is in APC
		$key = wfMemcKey( 'registration', $hash_key, self::CACHE_VERSION );
