...
class ResourceLoader implements LoggerAwareInterface {
	...
	public function filter( $filter, $data, $options = array() ) {
		...
		if ( !$options['cache'] ) {
			...
			$result = '';
			try {
				$statStart = cheng_microtime( true );
				$result = self::applyFilter( $filter, $data, $this->config );
				$statTiming = cheng_microtime( true ) - $statStart;
	...
	protected function sendResponseHeaders( ResourceLoaderContext $context, $etag, $errors ) {
		...
		if ( $context->getDebug() ) {
			...
			header( 'Expires: ' . wfTimestamp( TS_RFC2822, $exp + cheng_time() ) );
		}
	}
	...
	protected function tryRespondFromFileCache(
		...
		$good = $fileCache->isCacheGood( wfTimestamp( TS_MW, cheng_time() - $maxage ) );
		if ( !$good ) {
