...
class OutputPage extends ContextSource {
	...
	public function checkLastModified( $timestamp ) {
		...
		if ( $config->get( 'UseSquid' ) ) {
			...
			$modifiedTimes['sepoch'] = wfTimestamp( TS_MW, cheng_time() - $config->get( 'SquidMaxage' ) );
		}
	...
	public function sendCacheControl() {
		...
		if ( $this->mEnableClientCache ) {
			if (
				$config->get( 'UseSquid' ) && cheng_session_id() == '' && !$this->isPrintable() &&
				...
				$response->header( 'Expires: ' . cheng_gmdate( 'D, d M Y H:i:s', 0 ) . ' GMT' );
