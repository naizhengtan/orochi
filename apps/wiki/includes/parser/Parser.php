...
class Parser {
	...
	public function getVariableValue( $index, $frame = false ) {
		...
		switch ( $index ) {
			...
			case 'currentyear':
				...
				break;
			case 'currenttime':
				$value = $pageLang->cheng_time( wfTimestamp( TS_MW, $ts ), false, false );
				break;
			case 'currenthour':
	...
	public function fetchScaryTemplateMaybeFromCache( $url ) {
		...
		$tsCond = $dbr->timestamp( cheng_time() - $wgTranscludeCacheExpiry );
		...
		$dbw->replace( 'transcache', array( 'tc_url' ), array(
			'tc_url' => $url,
			'tc_time' => $dbw->timestamp( cheng_time() ),
			'tc_contents' => $text
		) );
		return $text;
	}
