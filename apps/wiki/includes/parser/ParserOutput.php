...
class ParserOutput extends CacheTime {
    ...
    //$text = preg_replace_callback( ParserOutput::EDITSECTION_REGEX, "tmp_callable", $text);
    //cheng_debug_lts("------".print_r($text,true)); $m=array(); preg_match(ParserOutput::EDITSECTION_REGEX, $text, $m); if(count($m)>0) {$text = tmp_callable($m);}cheng_debug_lts("------".print_r($text,true));exit(1);
    do{ $m=array(); preg_match(ParserOutput::EDITSECTION_REGEX, $text, $m); if(count($m)==0) break; $replace_text=tmp_callable($m);$text = preg_replace(ParserOutput::EDITSECTION_REGEX, $replace_text, $text,1);}while(true);
		} else {
...
//101
	 ...
	 *  - cpu: CPU cheng_time(requires getrusage)
	 *
	 * @since 1.22
	 * @param string $clock
	 * @return float|null
