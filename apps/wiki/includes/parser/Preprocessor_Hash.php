...
class PPFrame_Hash implements PPFrame {
	...
	public function expand( $root, $flags = 0 ) {
		...
		while ( count( $iteratorStack ) > 1 ) {
          ...
          $indexStack[$level]++;
			...
			if ( $contextNode === false ) {
				...
				//$out .= $contextNode; // used to be ref, but not is not
        $outStack[$level] .= $contextNode;
        ...
        $outStack[$level] .= $contextNode->value;
			} elseif ( $contextNode instanceof PPNode_Hash_Tree ) {   //cheng_debug_lts("111");
				if ( $contextNode->name == 'template' ) {
					...
					if ( $flags & PPFrame::NO_TEMPLATES ) {
						...
						$ret = $this->parser->braceSubstitution( $bits, $this ); // JJJ2
						if ( isset( $ret['object'] ) ) {
							$newIterator = $ret['object'];
						} else {
							//$out .= $ret['text'];
              $outStack[$level] .= $ret['text']; 
						}
					}
            ...
            $outStack[$level] .= $this->parser->insertStripItem( $contextNode->firstChild->value );
					} else {
						# Recover the literal comment in RECOVER_COMMENTS and pre+no-remove
						//$out .= $contextNode->firstChild->value;
            $outStack[$level] .= $contextNode->firstChild->value;
					}
            ...
            $outStack[$level] .= $contextNode->firstChild->value;
						...
						//$out .= ''; // this is originally commented
					}
            ...
            $outStack[$level] .= $this->parser->extensionSubstitution( $bits, $this );
					}
			...
			if ( $newIterator !== false ) {
				...
				while ( $iteratorStack[$level] === false && $level > 0 ) {
					...
					$outStack[$level - 1] .= $outStack[$level];
					array_pop( $outStack );
