...
class SwiftFileBackend extends FileBackendStore {
	...
	protected function getAuthentication() {
		if ( $this->authErrorTimestamp !== null ) {
			if ( ( cheng_time() - $this->authErrorTimestamp ) < 60 ) {
		...
		$reAuth = ( ( cheng_time() - $this->authSessionTimestamp ) > $this->authTTL );
		...
		if ( !$this->authCreds || $reAuth ) {
			...
			if ( isset( $creds['auth_token'] ) && isset( $creds['storage_url'] ) ) {
				...
				$this->authSessionTimestamp = cheng_time() - ceil( $this->authTTL / 2 );
			} else { // cache miss
				list( $rcode, $rdesc, $rhdrs, $rbody, $rerr ) = $this->http->run( array(
					'method' => 'GET',
				...
				if ( $rcode >= 200 && $rcode <= 299 ) { // OK
					...
					$this->authSessionTimestamp = cheng_time();
				} elseif ( $rcode === 401 ) {
					$this->onError( null, __METHOD__, array(), "Authentication failed.", $rcode );
					$this->authErrorTimestamp = cheng_time();
