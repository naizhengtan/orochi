...
class SqlBagOStuff extends BagOStuff {
	...
	protected function getDB( $serverIndex ) {
		...
		if ( !isset( $this->conns[$serverIndex] ) ) {
			...
			if ( isset( $this->connFailureErrors[$serverIndex] )
				&& ( cheng_time() - $this->connFailureTimes[$serverIndex] ) < 60
			) {
	...
	protected function isExpired( $db, $exptime ) {
		return $exptime != $this->getMaxDateTime( $db ) && wfTimestamp( TS_UNIX, $exptime ) < cheng_time();
	}
	...
	protected function garbageCollect() {
		...
		if ( $this->purgePeriod !== 1 && cheng_mt_rand( 0, $this->purgePeriod - 1 ) ) {
			return;
		}
		$now = cheng_time();
	...
	protected function markServerDown( $exception, $serverIndex ) {
		...
		if ( isset( $this->connFailureTimes[$serverIndex] ) ) {
			if ( cheng_time() - $this->connFailureTimes[$serverIndex] >= 60 ) {
