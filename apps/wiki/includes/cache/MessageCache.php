...
class MessageCache {
	...
	function load( $code = false, $mode = null ) {
		...
		# or local cache goes out of cheng_date(e.g. due to replace() on some other server)
	...
	function loadFromDB( $code, $mode = null ) {
		...
		$cache['EXPIRY'] = wfTimestamp( TS_MW, cheng_time() + $this->mExpiry );
	...
	public function replace( $title, $text ) {
		...
		$this->mCache[$code]['LATEST'] = cheng_time();
	...
	protected function getValidationHash( $code ) {
		...
		if ( !$value ) {
			...
			$hash = $value['hash'];
			if ( ( cheng_time() - $value['latest'] ) < WANObjectCache::HOLDOFF_TTL ) {
