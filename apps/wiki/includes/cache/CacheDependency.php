...
class FileDependency extends CacheDependency {
  ...
  private $shadow_filename;
    ...
    $this->shadow_filename = replace_abspath_to_placeholder($this->filename);

		return array( 'timestamp', 'shadow_filename');
  }

  function __wakeup() {
    $this->filename = replace_placeholder_to_abspath($this->shadow_filename);
  }
	...
	function loadDependencyValues() {
		if ( is_null( $this->timestamp ) ) {
			...
			$this->timestamp = cheng_filemtime( $this->filename );
	...
	function isExpired() {
		MediaWiki\suppressWarnings();
		$lastmod = cheng_filemtime( $this->filename );
		MediaWiki\restoreWarnings();
		if ( $lastmod === false ) {
...
class GlobalDependency extends CacheDependency {
  ...
  function __sleep() {
    $this->shadow_value = replace_abspath_to_placeholder($this->value);
    return array("name", "shadow_value");
  }

  function __wakeup() {
    $this->value = replace_placeholder_to_abspath($this->shadow_value);
  }
