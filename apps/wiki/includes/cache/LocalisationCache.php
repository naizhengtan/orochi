...
class LCStoreDB implements LCStore {
	...
	public function get( $code, $key ) {
		...
		if ( $row ) {
			return cheng_unserialize( $db->decodeBlob( $row->lc_value ) );
		} else {
			return null;
		}
	}
	...
	public function set( $key, $value ) {
		...
		$this->batch[] = array(
			...
			'lc_key' => $key,
			'lc_value' => $this->dbw->encodeBlob( cheng_serialize( $value ) ) );
	}
}
...
class LCStoreCDB implements LCStore {
	...
	public function get( $code, $key ) {
		...
		if ( !$this->readers[$code] ) {
			...
			return cheng_unserialize( $value );
		}
	}
	...
	public function set( $key, $value ) {
		...
		try {
			$this->writer->set( $key, cheng_serialize( $value ) );
		} catch ( CdbException $e ) {
			throw new MWException( $e->getMessage() );
		}
	}
...
class LocalisationCacheBulkLoad extends LocalisationCache {
	...
	protected function readPHPFile( $fileName, $fileType ) {
		...
		if ( !isset( $this->fileCache[$fileName][$fileType] ) ) {
			...
			return cheng_unserialize( $this->fileCache[$fileName][$fileType] );
		} else {
