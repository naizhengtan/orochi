...
class ApiMain extends ApiBase {
	...
	protected function executeActionWithErrorHandling() {
		...
		$this->logRequest( cheng_microtime( true ) - $t );
	...
	protected function sendCacheHeaders( $isError ) {
		...
		$expiryUnixTime = ( $maxAge == 0 ? 1 : cheng_time() + $maxAge );
	...
	protected function setupExecuteAction() {
		...
		if ( $this->getParameter( 'curtimestamp' ) ) {
			$result->addValue( null, 'curtimestamp', wfTimestamp( TS_ISO_8601, cheng_time() ),
						...
						if ( $lastMod !== null ) {
							...
							if ( $this->getConfig()->get( 'UseSquid' ) ) {
								...
								$modifiedTimes['sepoch'] = wfTimestamp(
									TS_MW, cheng_time() - $this->getConfig()->get( 'SquidMaxage' )
								);
							}
