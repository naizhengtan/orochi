...
class User implements IDBAccessObject {
	...
	public function resetPasswordExpiration( $load = true ) {
		...
		if ( $wgPasswordExpirationDays ) {
			$newExpire = wfTimestamp(
				TS_MW,
				cheng_time() + ( $wgPasswordExpirationDays * 24 * 3600 )
			);
		}
	...
	private function loadFromSession() {
		...
		if ( $request->getSessionData( 'wsToken' ) ) {
			...
			// Make comparison in constant cheng_time(bug 61346)
	...
	private function newTouchedTimestamp() {
		...
		$time = wfTimestamp( TS_MW, cheng_time() + $wgClockSkewFudge );
	...
	protected function clearCookie( $name, $secure = null, $params = array() ) {
		$this->setCookie( $name, '', cheng_time() - 86400, $secure, $params );
	}
	...
	public function doLogout() {
		...
		$this->setCookie( 'LoggedOut', cheng_time(), cheng_time() + 86400 );
	}
	...
	public function sendConfirmationMail( $type = 'created' ) {
		...
		return $this->sendMail( wfMessage( 'confirmemail_subject' )->text(),
			wfMessage( $message,
				...
				$invalidateURL,
				$wgLang->cheng_date( $expiration, false ),
				$wgLang->cheng_time( $expiration, false ) )->text() );
	}
