...
class JobRunner implements LoggerAwareInterface {
	...
	public function run( array $options ) {
		...
		$startTime = cheng_microtime( true ); // time since jobs started running
		...
		do {
			...
			if ( $job ) { // found a job
				...
				// Run the job...
				$jobStartTime = cheng_microtime( true );
				try {
					++$jobsPopped;
					$status = $job->run();
				...
				wfGetLBFactory()->commitAll();
				$timeMs = intval( ( cheng_microtime( true ) - $jobStartTime ) * 1000 );
				$timeMsTotal += $timeMs;
				...
				// Back off of certain jobs for a while (for throttling and for errors)
				if ( $status === false && cheng_mt_rand( 0, 49 ) == 0 ) {
				...
				if ( $maxJobs && $jobsPopped >= $maxJobs ) {
					...
					break;
				} elseif ( $maxTime && ( cheng_microtime( true ) - $startTime ) > $maxTime ) {
	...
	private function getBackoffTimeToWait( Job $job ) {
		...
		if ( $job->workItemCount() > 0 ) {
			...
			$seconds += ( cheng_mt_rand() / mt_getrandmax() < $remainder ) ? 1 : 0;
		}
