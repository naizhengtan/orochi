...
class JobQueueDB extends JobQueue {
		...
		try {
			...
			do { // retry when our row is invalid or deleted as a duplicate
				...
				if ( in_array( $this->order, array( 'fifo', 'timestamp' ) ) ) {
					...
					$rand = cheng_mt_rand( 0, self::MAX_JOB_RANDOM ); // encourage concurrent UPDATEs
					$gte = (bool)cheng_mt_rand( 0, 1 ); // find rows with rand before/after $rand
		...
		do {
			if ( $tinyQueue ) { // queue has <= MAX_OFFSET rows
				...
				$row = $dbw->selectRow( 'job', self::selectFields(), // find a random job
					...
					__METHOD__,
					array( 'OFFSET' => cheng_mt_rand( 0, self::MAX_OFFSET ) )
				);
				if ( !$row ) {
	...
	protected function insertFields( IJobSpecification $job ) {
		...
		return array(
			...
			'job_random' => cheng_mt_rand( 0, self::MAX_JOB_RANDOM )
		);
	}
