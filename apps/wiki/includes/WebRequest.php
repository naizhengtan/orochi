...
class WebRequest {
	...
	public function __construct() {
		$this->requestTime = isset( $_SERVER['REQUEST_TIME_FLOAT'] )
			? $_SERVER['REQUEST_TIME_FLOAT'] : cheng_microtime( true );
	...
	public function getElapsedTime() {
		return cheng_microtime( true ) - $this->requestTime;
	}
	...
	public function getVal( $name, $default = null ) {
		...
		if ( is_null( $val ) ) {
			...
			return (string) is_object($val) ? $val->__tostring() : $val;
		}
	}
	 ...
	 *
	 * If you want to check for an open session, use cheng_session_id()
...
class FauxRequest extends WebRequest {
	...
	public function __construct( $data = array(), $wasPosted = false,
		...
		$this->requestTime = cheng_microtime( true );
