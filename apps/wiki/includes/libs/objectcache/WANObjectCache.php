...
class WANObjectCache {
	...
	final public function delete( $key, $ttl = self::HOLDOFF_TTL ) {
		...
		$ok = $this->cache->set( $key, self::PURGE_VAL_PREFIX . cheng_microtime( true ), $ttl );
	...
	final public function getCheckKeyTime( $key ) {
		...
		if ( $time === false ) {
			...
			$time = (string)cheng_microtime( true );
	...
	final public function touchCheckKey( $key ) {
		...
		$ok = $this->cache->set( $key,
			self::PURGE_VAL_PREFIX . cheng_microtime( true ), self::CHECK_KEY_TTL );
		// Publish the purge to all clusters
	...
	protected function worthRefresh( $curTTL, $lowTTL ) {
		...
		return cheng_mt_rand( 1, 1e9 ) <= 1e9 * $chance;
	}
	...
	protected function wrap( $value, $ttl ) {
		return array(
			...
			self::FLD_TTL => $ttl,
			self::FLD_TIME => cheng_microtime( true )
		);
	}
