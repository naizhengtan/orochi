...
class APCBagOStuff extends BagOStuff {
	...
	public function __construct( array $params = array() ) {
		...
		if ( isset( $params['nativeSerialize'] ) ) {
			...
			$this->nativeSerialize = /*true*/false; //cheng-hack: we need serialization for the cache during replay
		}
	}
	...
	public function get( $key, &$casToken = null, $flags = 0 ) {
        ...
        $val = '';
        if ( $lingfan_use_modified_apc ) {
            $val = lingfan_apc_fetch( $key . self::KEY_SUFFIX );
        } else {
            $val = oro_apc_fetch( $key . self::KEY_SUFFIX );
        }

		$casToken = $val;
//global $req_no;
//cheng_debug_lts("[$req_no] hash=".get_cf_hash()."  key=[$key], val=[$val]");
		if ( is_string( $val ) && !$this->nativeSerialize ) {
			$val = $this->isInteger( $val )
				? intval( $val )
				: cheng_unserialize( $val );
		}
	...
	public function set( $key, $value, $exptime = 0 ) {
		...
		if ( !$this->nativeSerialize && !$this->isInteger( $value ) ) {
			$value = cheng_serialize( $value );
		}

        if ( $lingfan_use_modified_apc ) {
            lingfan_apc_store($key . self::KEY_SUFFIX, $value, $exptime );
        } else {
    		oro_apc_store( $key . self::KEY_SUFFIX, $value, $exptime );
        }
	...
	public function delete( $key ) {
        global $lingfan_use_modified_apc;
        if ( $lingfan_use_modified_apc ) {
            lingfan_apc_store($key . self::KEY_SUFFIX, NULL, 0, 2 );
        } else {
    		oro_apc_delete( $key . self::KEY_SUFFIX );
        }
	...
	public function incr( $key, $value = 1 ) {
        //FIXME: not supported
        //$dump = var_export(debug_backtrace(10), true);
        //lingfan_print_log("apc_inc\nkey: $key value: $value\n$dump");
		return oro_apc_inc( $key . self::KEY_SUFFIX, $value );
	}

	public function decr( $key, $value = 1 ) {
        //FIXME: not supported
        //$dump = var_export(debug_backtrace(10), true);
        //lingfan_print_log("apc_dec\nkey: $key value: $value\n$dump");
		return oro_apc_dec( $key . self::KEY_SUFFIX, $value );
	}
}
