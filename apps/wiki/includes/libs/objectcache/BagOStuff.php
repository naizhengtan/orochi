...
abstract class BagOStuff implements LoggerAwareInterface {
	...
	public function lock( $key, $timeout = 6, $expiry = 6, $rclass = '' ) {
		...
		$this->clearLastError();
		$timestamp = cheng_microtime( true ); // starting UNIX timestamp

    // 

    // for shrink control flow group, we provide a naive native lock for this
    $lock_aval_and_acq = cheng_lock($key);

    if ($lock_aval_and_acq) {
      $locked = true;
    } else {

    // Orignial APC-based lock implementation
		if ( $this->add( "{$key}:lock", 1, $expiry ) ) {
			$locked = true;
		} elseif ( $this->getLastError() || $timeout <= 0 ) {
			$locked = false; // network partition or non-blocking
		} else {
			$uRTT = ceil( 1e6 * ( cheng_microtime( true ) - $timestamp ) ); // estimate RTT (us)
			...
			} while ( !$locked && ( cheng_microtime( true ) - $timestamp ) < $timeout );
		}
    }
	...
	public function unlock( $key ) {
      ...
      $lock_aval_and_rels = cheng_unlock($key);

      if (!$lock_aval_and_rels) {
        // Orignial APC-based lock implementation
	...
	final public function getScopedLock( $key, $timeout = 6, $expiry = 30, $rclass = '' ) {
		...
		$lSince = cheng_microtime( true ); // lock timestamp
		...
		return new ScopedCallback( function() use ( $that, $logger, $key, $lSince, $expiry ) {
			...
			$age = ( cheng_microtime( true ) - $lSince + $latency );
