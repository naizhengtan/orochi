...
class DatabaseMysqli extends DatabaseMysqlBase {
	...
	protected function doQuery( $sql ) {
        ...
        //lingfan_print_log("$sql\n$dump\n", "/tmp/access.log");
		$conn = $this->getBindingHandle();

		if ( $this->bufferResults() ) {
			//$ret = $conn->query( $sql );
			$ret = oro_mysql_query( $conn, $sql );
		} else {
			//$ret = $conn->query( $sql, MYSQLI_USE_RESULT );
			$ret = oro_mysql_query( $conn, $sql, MYSQLI_USE_RESULT );
		}

		return $ret;
    }

    public function lingfan_doQuery( $sql ) {
    ...
    return cheng_mysqli_affected_rows($conn);
	}
