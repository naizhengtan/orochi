...
abstract class DatabaseBase implements IDatabase {
	...
	public function query( $sql, $fname = __METHOD__, $tempIgnore = false ) {
		...
		if ( $isWriteQuery ) {
			...
			$this->mDoneWrites = cheng_microtime( true );
		}
		...
		if ( !$this->mTrxLevel && $this->getFlag( DBO_TRX ) && $this->isTransactableQuery( $sql ) && !$lingfan_called_it ) {
			if ( $wgDebugDBTransactions ) {
		...
		$queryRuntime = cheng_microtime( true ) - $startTime;
		...
		do {
			try {
				...
				if ( $this->wasDeadlock() ) {
					...
					usleep( cheng_mt_rand( self::DEADLOCK_DELAY_MIN, self::DEADLOCK_DELAY_MAX ) );
				} else {
	...
	final public function begin( $fname = __METHOD__ ) {
		...
		$this->doBegin( $fname );
		$this->mTrxTimestamp = cheng_microtime( true );
...
//            lingfan_print_log("commit\n$dump\n", "/tmp/access.log");
	...
	protected function doRollback( $fname ) {
		if ( $this->mTrxLevel ) {
//            lingfan_print_log("rollback", "/tmp/access.log");
