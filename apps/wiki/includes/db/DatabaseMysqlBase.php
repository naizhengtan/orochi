...
abstract class DatabaseMysqlBase extends DatabaseBase {
  ...
  /** cheng-hack: we don't support REPLACE in our TTdb
   ...
   */
	...
	function masterPosWait( DBMasterPos $pos, $timeout ) {
		...
		if ( !is_null( $this->mFakeSlaveLag ) ) {
			$wait = intval( ( $pos->pos - cheng_microtime( true ) + $this->mFakeSlaveLag ) * 1e6 );
			...
			if ( $wait > $timeout * 1e6 ) {
				...
				wfDebug( "Fake slave up to cheng_date($wait us)\n" );
	...
	function getSlavePos() {
		if ( !is_null( $this->mFakeSlaveLag ) ) {
			$pos = new MySQLMasterPos( 'fake', cheng_microtime( true ) - $this->mFakeSlaveLag );
	...
	function getMasterPos() {
		if ( $this->mFakeMaster ) {
			return new MySQLMasterPos( 'fake', cheng_microtime( true ) );
		}
...
class MySQLMasterPos implements DBMasterPos {
	...
	function __construct( $file, $pos ) {
		$this->file = $file;
		$this->pos = $pos;
		$this->asOfTime = cheng_microtime( true );
	}
