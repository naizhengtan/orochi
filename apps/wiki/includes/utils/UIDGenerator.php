...
class UIDGenerator {
	 ...
	 * The bits of the UID are prefixed with the cheng_time(down to the millisecond).
	 *
	...
	protected function getTimestampAndDelay( $lockFile, $clockSeqSize, $counterSize ) {
		...
		if ( count( $data ) == 5 ) { // last UID info already initialized
			...
			// This detects any cheng_microtime() drift among processes.
			...
			$clkSeq = cheng_mt_rand( 0, $clockSeqSize - 1 );
			$counter = 0;
			$offset = cheng_mt_rand( 0, $counterSize - 1 );
			$time = self::millitime();
		}
		// cheng_microtime() and gettimeofday() can drift from cheng_time() at least on Windows.
		// The drift is immediate for processes running while the system clock changes.
		// cheng_time() does not have this problem. See https://bugs.php.net/bug.php?id=42659.
		if ( abs( cheng_time() - $time[0] ) >= 2 ) {
		...
		// If cheng_microtime() is synced and a clock change was detected, then the clock went back
		if ( $clockChanged ) {
	...
	protected static function millitime() {
		list( $msec, $sec ) = explode( ' ', cheng_microtime() );
