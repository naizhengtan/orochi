...
class MediaWiki {
    ...
    if ( !$context ) {
			$context = RequestContext::getMain(); // Create static RequestContext object
		}
	...
	private function main() {
		global $wgTitle, $wgTrxProfilerLimits, $lingfan_called_it;
      ...
      // Set a dummy title, because $wgTitle == null might break things
        ...
        $lingfan_url = $request->getVal( 'title' );
        $lingfan_called_it = true;
        $lingfan_options = new ParserOptions( $this->context->getUser(), Language::factory( "en" ) );
        $lingfan_called_it = false;

        lingfan_apc_init( $lingfan_options, $lingfan_url );

        // ********************lingan's change end**************************

		// Get title from request parameters,
		// is set on the fly by parseTitle the first time.
		$title = $this->getTitle(); wfDebug("Enter getAction()");

		$action = $this->getAction(); wfDebug("Exit getAction()");
		$wgTitle = $title;
    ...
    // Output everything!
        ...
        // ******************lingfan's change starts************************
        lingfan_apc_flush();
        //lingfan_print_log("=================lingfan end of request===============\n", "/tmp/access.log");
        // ********************lingan's change end**************************
	}
	...
	public function triggerJobs() {
		...
		if ( $jobRunRate < 1 ) {
			$max = mt_getrandmax();
			if ( cheng_mt_rand( 0, $max ) > $max * $jobRunRate ) {
		...
		$query = array( 'title' => 'Special:RunJobs',
			'tasks' => 'jobs', 'maxjobs' => $n, 'sigexpiry' => cheng_time() + 5 );
