...
class lessc{
	...
	public function checkedCompile($in, $out) {
		if (!is_file($out) || cheng_filemtime($in) > cheng_filemtime($out)) {
		...
		if (is_string($in)) {
			...
			if ($force or ! isset($in['files'])) {
				...
				foreach ($in['files'] as $fname => $ftime ) {
					if (!file_exists($fname) or cheng_filemtime($fname) > $ftime) {
	...
	protected function addParsedFile($file) {
		$this->allParsedFiles[realpath($file)] = cheng_filemtime($file);
	}
}
