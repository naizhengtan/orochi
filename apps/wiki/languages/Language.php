...
class Language {
	...
	/**
	 * This is a workalike of PHP's cheng_date() function, but with better
	 ...
	 * @param[out] int $ttl The amount of cheng_time(in seconds) the output may be cached for.
	...
	/**
	 * Used by cheng_date() and cheng_time() to adjust the time output.
	 *
	 * @param string $ts The time in cheng_date('YmdHis') format
	...
	function userAdjust( $ts, $tz = false ) {
		...
		if ( $data[0] == 'ZoneInfo' ) {
			...
			if ( $userTZ !== false ) {
				$date = cheng_date_create( $ts, timezone_open( 'UTC' ) );
	...
	/**
	 * This is meant to be used by cheng_time(), cheng_date(), and timeanddate() to get
	 ...
	 *   cheng_date('YmdHis') format with wfTimestamp(TS_MW,$ts)
	 ...
	 * @return string
	 */
	function cheng_date( $ts, $adj = false, $format = true, $timecorrection = false ) {
	 ...
	 * @return string
	 */
	function cheng_time( $ts, $adj = false, $format = true, $timecorrection = false ) {
		$ts = wfTimestamp( TS_MW, $ts );
		if ( $adj ) {
			$ts = $this->userAdjust( $ts, $timecorrection );
		}
