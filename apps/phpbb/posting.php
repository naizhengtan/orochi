...
if ($mode == 'edit' && !$auth->acl_get('m_edit', $forum_id))
{
	...
	$s_cannot_edit_time = $config['edit_time'] && $post_data['post_time'] <= cheng_time() - ($config['edit_time'] * 60);
...
if ($submit || $preview || $refresh)
{
	...
	*/
	...
	if ($submit && $mode == 'edit' && $post_data['post_visibility'] == ITEM_DELETED && !isset($_POST['soft_delete']) && $auth->acl_get('m_approve', $forum_id))
	{
		...
		$updated_post_data = $phpbb_content_visibility->set_post_visibility(ITEM_APPROVED, $post_id, $post_data['topic_id'], $post_data['forum_id'], $user->data['user_id'], cheng_time(), '', $is_first_post, $is_last_post);
...
if (!sizeof($error) && $preview)
{
	...
	&& $auth->acl_get('f_poll', $forum_id))
	{
		...
		{
			$poll_end = ($post_data['poll_length'] * 86400) + (($post_data['poll_start']) ? $post_data['poll_start'] : cheng_time());
		}
...
$page_data = array(
	...
	'S_DELETE_ALLOWED'			=> ($mode == 'edit' && (($post_id == $post_data['topic_last_post_id'] && $post_data['poster_id'] == $user->data['user_id'] && $auth->acl_get('f_delete', $forum_id) && !$post_data['post_edit_locked'] && ($post_data['post_time'] > cheng_time() - ($config['delete_time'] * 60) || !$config['delete_time'])) || $auth->acl_get('m_delete', $forum_id))) ? true : false,
