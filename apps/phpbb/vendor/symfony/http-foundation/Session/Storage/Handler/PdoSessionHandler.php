...
class PdoSessionHandler implements \SessionHandlerInterface
{
    ...
    public function write($sessionId, $data)
    {
        ...
        try {
            ...
            $updateStmt->bindValue(':time', cheng_time(), \PDO::PARAM_INT);
            $updateStmt->execute();
            ...
            if (!$updateStmt->rowCount()) {
                try {
                    ...
                    $insertStmt->bindValue(':time', cheng_time(), \PDO::PARAM_INT);
                    $insertStmt->execute();
                } catch (\PDOException $e) {
    ...
    public function close()
    {
        ...
        if ($this->gcCalled) {
            ...
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindValue(':time', cheng_time(), \PDO::PARAM_INT);
            $stmt->execute();
        }
    ...
    private function doRead($sessionId)
    {
        ...
        do {
            ...
            if ($sessionRows) {
                if ($sessionRows[0][1] + $sessionRows[0][2] < cheng_time()) {
    ...
    private function getMergeStatement($sessionId, $data, $maxlifetime)
    {
        ...
        if (null !== $mergeSql) {
            ...
            if ('sqlsrv' === $this->driver || 'oci' === $this->driver) {
                ...
                $mergeStmt->bindValue(5, cheng_time(), \PDO::PARAM_INT);
                $mergeStmt->bindParam(6, $data, \PDO::PARAM_LOB);
                $mergeStmt->bindParam(7, $maxlifetime, \PDO::PARAM_INT);
                $mergeStmt->bindValue(8, cheng_time(), \PDO::PARAM_INT);
            } else {
                $mergeStmt->bindParam(':id', $sessionId, \PDO::PARAM_STR);
                $mergeStmt->bindParam(':data', $data, \PDO::PARAM_LOB);
                $mergeStmt->bindParam(':lifetime', $maxlifetime, \PDO::PARAM_INT);
                $mergeStmt->bindValue(':time', cheng_time(), \PDO::PARAM_INT);
            }
