...
if ($mark_read == 'topics')
{
	...
	if ($request->is_ajax())
	{
		...
		$data = array(
			...
			'U_MARK_TOPICS'		=> ($user->data['is_registered'] || $config['load_anon_lastread']) ? append_sid("{$phpbb_root_path}viewforum.$phpEx", 'hash=' . generate_link_hash('global') . "&f=$forum_id&mark=topics&mark_time=" . cheng_time()) : '',
