	...
	*/
	function session_create($user_id = false, $set_admin = false, $persist_login = false, $viewonline = true)
	{
		...
		*/
		...
		{
			$this->data['session_last_visit'] = (isset($this->data['session_time']) && $this->data['session_time']) ? $this->data['session_time'] : (($this->data['user_lastvisit']) ? $this->data['user_lastvisit'] : cheng_time());
		}
		else
		{
...
//		$db->sql_return_on_error(false);
    ...
    // cheng-hack: lower the guest user's update to session_table to 60s
    $last_insert_id = "_LAST_INSERT_USR_".$this->data['user_id'];
    $last_insert_time = $cache->get_driver()->_read($last_insert_id);
    $last_insert_counter_id = "_LAST_INSERT_COUNTER_USR_".$this->data['user_id'];
    $last_insert_counter = $cache->get_driver()->_read($last_insert_counter_id);

    // only update when last update is 60s ago
    if (!$last_insert_time || !$last_insert_counter || $last_insert_counter < 2
      || (cheng_microtime(true) - $last_insert_time) > 60) {
      $sql = 'INSERT INTO ' . SESSIONS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
      $db->sql_query($sql);
		  $db->sql_return_on_error(false);
      $cache->get_driver()->_write($last_insert_id, cheng_microtime(true));

      if ((cheng_microtime(true) - $last_insert_time) > 60) {
        $last_insert_counter = 1;
      } else {
        $last_insert_counter++;
      }
      $cache->get_driver()->_write($last_insert_counter_id, $last_insert_counter);
    }
		...
		if (!$bot)
		{
			...
			if ((int) $row['sessions'] <= 1 || empty($this->data['user_form_salt']))
			{
				...
				$db->sql_query($sql);
        $last_insert_counter_id = "_LAST_INSERT_COUNTER_USR_".$this->data['user_id'];
        $cache->get_driver()->_write($last_insert_counter_id, 0);
			}
		}
		else
		{
	...
	function session_kill($new_session = true)
	{
		...
		*/
		...
		if ($this->data['user_id'] != ANONYMOUS)
		{
			...
			{
				$this->data['session_time'] = cheng_time();
			}
	...
	*/
	function session_gc()
	{
		...
		if ($del_sessions < $batch_size)
		{
			...
			{
				$sql = 'DELETE FROM ' . SESSIONS_KEYS_TABLE . '
					WHERE last_login < ' . (cheng_time() - (86400 * (int) $config['max_autologin_time']));
				$db->sql_query($sql);
			}
			...
			$sql = 'DELETE FROM ' . LOGIN_ATTEMPT_TABLE . '
				WHERE attempt_time < ' . (cheng_time() - (int) $config['ip_login_limit_time']);
			$db->sql_query($sql);
		}
	...
	*/
	function set_cookie($name, $cookiedata, $cookietime, $httponly = true)
	{
		...
		$expire = cheng_gmdate('D, d-M-Y H:i:s \\G\\M\\T', $cookietime);
	...
	*/
	function check_ban($user_id = false, $user_ips = false, $user_email = false, $return = false)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			if ($row['ban_end'] && $row['ban_end'] < cheng_time())
			{
				continue;
			}
...
//				$this->session_create(ANONYMOUS);
	...
	*/
	...
	{
		global $db, $phpbb_dispatcher, $cache;

		$session_id = ($session_id) ? $session_id : $this->session_id;

    // cheng-hack: check the last update time
    $last_update_id = "_LAST_UPDATE_".$session_id;
    $last_update_time = $cache->get_driver()->_read($last_update_id);

    // only update when last update is 60s ago
    if (!$last_update_time || (cheng_microtime(true) - $last_update_time) > 60) {
