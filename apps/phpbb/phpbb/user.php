...
class user extends \phpbb\session
{
	...
	*/
	function setup($lang_set = false, $style_id = false)
	{
      ...
      // cheng-hack:
		  ...
		  $this->style[$key] = cheng_settype($this->style[$key], gettype($default_value));
		...
		*/
		...
		if (!defined('IN_ADMIN') && !defined('ADMIN_START') && $config['chg_passforce'] && !empty($this->data['is_registered']) && $auth->acl_get('u_chgpasswd') && $this->data['user_passchg'] < cheng_time() - ($config['chg_passforce'] * 86400))
		{
	...
	* @param string $format date format in cheng_date() notation. | used to indicate relative dates, for example |d m Y|, h:i is translated to Today, h:i.
	...
	*/
	function format_date($gmepoch, $format = false, $forcedate = false)
	{
		...
		return $time->format($format, $forcedate); // XXX: the problem is $time is non-determinism
	}
