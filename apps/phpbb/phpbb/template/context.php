...
class context
{
    ...
    // cheng-hack:
      ...
      // cheng-hack: hate ref
			foreach ($ref as $loop_name => $loop_data)
			{
				...
				$loop_data = $this->set_num_rows($loop_data);
        $ref[$loop_name] = $loop_data;
			}
			$this->num_rows_is_set = true;
		}

    $this->tpldata = $ref;
    $ret_ref = &$this->tpldata;
	...
	*/
	//protected function cheng_set_num_rows($loop_data)
	protected function set_num_rows($loop_data)
	{
		$s_num_rows = sizeof($loop_data);
    // cheng-hack: I hate ref!!!
		foreach ($loop_data as $k=>$mod_block)
		{
			foreach ($mod_block as $sub_block_name => $sub_block)
			{
				...
				{
					$sub_block = $this->set_num_rows($sub_block);
          $mod_block[$sub_block_name] = $sub_block;
				}
			}
  ...
  // cheng-hack
	//public function cheng_assign_block_vars($blockname, array $vararray)
	public function assign_block_vars($blockname, array $vararray)
	{
      ...
      $keys = array();
        ...
        $tmp_keys = $keys;
        $tmp_keys[] = $blocks[$blockcount];
        $tmp_keys[] = ($s_row_count - 1);
        $tmp_keys[] = 'S_LAST_ROW';
				$this->tpldata = cheng_unset($this->tpldata, $tmp_keys); 
			}

      // used to be a bug: duplicated empty reply problem
			//$str[$blocks[$blockcount]][] = $vararray;
      $keys[] = $blocks[$blockcount];
      //cheng_debug_lts("keys---".print_r($keys,true));
      //cheng_debug_lts("tpldata-before--".print_r($this->tpldata,true));
      $this->tpldata = cheng_append($this->tpldata, $keys, $vararray);
      //cheng_debug_lts("tpldata-after--".print_r($this->tpldata,true));
		}
		else
		{
			// Top-level block.
	...
	*/
	//public function cheng_alter_block_array($blockname, array $vararray, $key = false, $mode = 'insert')
    ...
    $keys = array();
      ...
      // cheng: $block above has been changed!!!!
      $this->tpldata = cheng_assign($this->tpldata, $keys, $block);

      //cheng_debug_lts("==");
      //cheng_debug_lts("==".print_r($this->tpldata,true));
	...
	* @return true
	*/
	//public function cheng_destroy_block_vars($blockname)
	public function destroy_block_vars($blockname)
	{
      ...
      $this->tpldata = cheng_unset($this->tpldata, $keys);
		}
		else
		{
			// Top-level block.
...
//================orignal ref impl=======================
	public function orig_assign_block_vars($blockname, array $vararray)
	//public function assign_block_vars($blockname, array $vararray)
	{
		$this->num_rows_is_set = false;
		if (strpos($blockname, '.') !== false)
  ...
  //public function destroy_block_vars($blockname)
  public function orig_destroy_block_vars($blockname)
	{
		$this->num_rows_is_set = false;
		if (strpos($blockname, '.') !== false)
		{
	...
	//public function alter_block_array($blockname, array $vararray, $key = false, $mode = 'insert')
	public function orig_alter_block_array($blockname, array $vararray, $key = false, $mode = 'insert')
	{
		$this->num_rows_is_set = false;
		if (strpos($blockname, '.') !== false)
		{
	...
	//protected function set_num_rows(&$loop_data)
	protected function orig_set_num_rows(&$loop_data)
	{
		$s_num_rows = sizeof($loop_data);
