	...
	*/
	public function keyword_search($type, $fields, $terms, $sort_by_sql, $sort_key, $sort_dir, $sort_days, $ex_fid_ary, $post_visibility, $topic_id, $author_ary, $author_name, &$id_ary, &$start, $per_page)
	{
		...
		*/
		...
		if ($sort_days)
		{
			$sql_where[] = 'p.post_time >= ' . (cheng_time() - ($sort_days * 86400));
		}
	...
	*/
	public function author_search($type, $firstpost_only, $sort_by_sql, $sort_key, $sort_dir, $sort_days, $ex_fid_ary, $post_visibility, $topic_id, $author_ary, $author_name, &$id_ary, &$start, $per_page)
	{
		...
		*/
		...
		$sql_time		= ($sort_days) ? ' AND p.post_time >= ' . (cheng_time() - ($sort_days * 86400)) : '';
...
//					$select = 'SQL_CALC_FOUND_ROWS ' . $select;
	...
	*/
	public function index($mode, $post_id, &$message, &$subject, $poster_id, $forum_id)
	{
		...
		foreach ($words['add'] as $word_in => $word_ary)
		{
			...
			if (sizeof($word_ary))
			{
        // cheng-hack: tweak for sql parser
        // origin:
        /*
        ...
        */
        ...
        // cheng-hack: after
				$sql = 'SELECT  word_id FROM ' . SEARCH_WORDLIST_TABLE . '
					WHERE ' . $this->db->sql_in_set('word_text', $word_ary);
				$ret = $this->db->sql_query($sql);
			  $row = $this->db->sql_fetchrow($ret);
        $word_id = $row['word_id'];
			  $this->db->sql_freeresult($ret);

				$sql = 'INSERT INTO ' . SEARCH_WORDMATCH_TABLE . ' (post_id, word_id, title_match)
					VALUES (' . (int) $post_id . ', ' . $word_id . ', ' . (int) $title_match . ')';
				$this->db->sql_query($sql);
        // end
	...
	*/
	public function tidy()
	{
		...
		if (!$this->config['fulltext_native_load_upd'])
		{
			$this->config->set('search_last_gc', cheng_time(), false);
			return;
		}
