...
class migrate extends \phpbb\console\command\db\migration_command
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$this->migrator->set_output_handler(new log_wrapper_migrator_output_handler($this->language, new console_migrator_output_handler($this->user, $output), $this->phpbb_root_path . 'store/migrations_' . cheng_time() . '.log', $this->filesystem));
		...
		{
			$this->log->add('admin', ANONYMOUS, '', 'LOG_UPDATE_DATABASE', cheng_time(), array($orig_version, $this->config['version']));
		}
