...
class helper
{
	...
	*/
	public function unique_id($extra = 'c')
	{
		...
		$val = $this->config['rand_seed'] . cheng_microtime();
		$val = md5($val);
		$this->config['rand_seed'] = md5($this->config['rand_seed'] . $val . $extra);

		if ($dss_seeded !== true && ($this->config['rand_seed_last_update'] < cheng_time() - rand(1,10)))
		{
			$this->config->set('rand_seed_last_update', cheng_time(), true);
