...
class colour_manager
{
	...
	function random_colour($params = array(), $mode = false)
	{
		...
		switch ($mode)
		{
			...
			default:
				...
				$h = cheng_mt_rand($min_hue, $max_hue);
				$s = cheng_mt_rand($min_saturation, $max_saturation);
				$v = cheng_mt_rand($min_value, $max_value);
	...
	*/
	function mono_range($resource, $count = 5, $include_original = true)
	{
		...
		while ($count > 0)
		{
			$colour[1] = ($colour[1] + cheng_mt_rand(40,60)) % 99;
			$colour[2] = ($colour[2] + cheng_mt_rand(40,60));
