...
class file extends \phpbb\cache\driver\base
{
	...
	*/
	function tidy()
	{
		...
		$config->set('cache_last_gc', cheng_time(), false);
	}
	...
	*/
	function put($var_name, $var, $ttl = 31536000)
	{
		if ($var_name[0] == '_')
		{
			$this->_write('data' . $var_name, $var, cheng_time() + $ttl);
		}
		else
		{
			$this->vars[$var_name] = $var;
			$this->var_expires[$var_name] = cheng_time() + $ttl;
	...
	*/
	function _exists($var_name)
	{
		...
		else
		{
			...
			return (cheng_time() > $this->var_expires[$var_name]) ? false : isset($this->vars[$var_name]);
		}
	}
	...
	*/
	function sql_save(\phpbb\db\driver\driver_interface $db, $query, $query_result, $ttl)
	{
		...
		if ($this->_write('sql_' . $query_id, $this->sql_rowset[$query_id], $ttl + cheng_time(), $query))
		{
			return $query_id;
		}
