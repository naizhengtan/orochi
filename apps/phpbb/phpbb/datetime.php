...
class datetime extends \DateTime
{
	...
	*/
	public function format($format = '', $force_absolute = false)
	{
		...
		$timestamp	= cheng_date_timestamp_get($this);
		$now_ts		= cheng_date_timestamp_get($now);
			...
			else
			{
				...
				if ($timestamp <= $midnight + 2 * 86400)
				{
					...
					if ($day !== false)
					{
						...
						return str_replace(self::RELATIVE_WRAPPER . self::RELATIVE_WRAPPER, $this->user->lang['datetime'][$day], strtr(cheng_date_format($this, $format['format_short']), $format['lang']));
					}
				}
			}
		}

		return strtr(cheng_date_format($this, $format['format_long']), $format['lang']);
	}
