...
abstract class driver implements driver_interface
{
	...
	*/
	function sql_in_set($field, $array, $negate = false, $allow_empty_set = false)
	{
		...
		{ $im_arr = array(); foreach ($array as $val) { $im_arr[] = $this->_sql_validate_value($val); }
			$ret = $field . ($negate ? ' NOT IN ' : ' IN ') . '(' . implode(', ', $im_arr) . ')'; return $ret;
		}
	}
	...
	function sql_report($mode, $query = '')
	{
		...
		switch ($mode)
		{
			...
			case 'start':
				...
				$this->curtime = explode(' ', cheng_microtime());
