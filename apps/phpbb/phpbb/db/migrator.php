	...
	*/
	protected function try_apply($name)
	{
		...
		if (!isset($this->migration_state[$name]))
		{
			...
			{
				$state['migration_start_time'] = cheng_time();
			}
		}
		...
		if (!$state['migration_schema_done'])
		{
			...
			$elapsed_time = cheng_microtime(true);

			$steps = $this->helper->get_schema_steps($migration->update_schema());
			$result = $this->process_data_step($steps, $state['migration_data_state']);

			$elapsed_time = cheng_microtime(true) - $elapsed_time;
		...
		else if (!$state['migration_data_done'])
		{
			try
			{
				...
				$state['migration_end_time'] = ($result === true) ? cheng_time() : 0;
		...
		foreach ($migrations as $name)
		{
			if ($this->migration_state($name) === false)
			{
				$state = array(
					...
					'migration_data_done'	=> true,
					'migration_data_state'	=> '',
					'migration_start_time'	=> cheng_time(),
					'migration_end_time'	=> cheng_time(),
				);
				$this->set_migration_state($name, $state);
			}
		}
