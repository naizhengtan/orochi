...
class mysql_extractor extends base_extractor
{
	...
	*/
	public function write_start($table_prefix)
	{
		...
		$sql_data .= "# DATE : " . cheng_gmdate("d-m-Y H:i:s", $this->time) . " GMT\n";
		$sql_data .= "#\n";
		$this->flush($sql_data);
	}
	...
	*/
	protected function write_data_mysqli($table_name)
	{
		...
		$sql = "SELECT *
			FROM $table_name";
    // FIXME: not support MYSQLI_USE_RESULT yet!!!!
    echo "ERROR!ERROR!ALERT!ALERT!\n"
		$result = oro_mysql_query($this->db->get_db_connect_id(), $sql, MYSQLI_USE_RESULT);
		if ($result != false)
		{
