...
class db extends \phpbb\config\config
{
	...
	*/
	public function __construct(\phpbb\db\driver\driver_interface $db, \phpbb\cache\driver\driver_interface $cache, $table)
	{
      ...
      // cheng-hack: a transalation of deterministic sequence of query results
      $deterministic_mapping = array();
      while ($row = $this->db->sql_fetchrow($result)) {
        $deterministic_mapping[serialize($row)] = $row;
      }
      $deterministic_mapping = cheng_ksort($deterministic_mapping);
			foreach ($deterministic_mapping as $row) {
				if (!$row['is_dynamic']) {
