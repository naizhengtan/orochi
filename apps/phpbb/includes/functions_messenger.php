...
class messenger
{
	...
	*/
	function generate_message_id()
	{
		...
		return md5(unique_id(cheng_time())) . '@' . $domain;
	}
	...
	*/
	function build_header($to, $cc, $bcc)
	{
		...
		$headers[] = 'Date: ' . cheng_date('r', cheng_time());
...
class queue
{
	...
	*/
	function process()
	{
		...
		if (!$have_cache_file || $config['last_queue_run'] > cheng_time() - $config['queue_interval'])
		{
			if (!$have_cache_file)
			{
				$config->set('last_queue_run', cheng_time(), false);
			}
