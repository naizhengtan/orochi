	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			case 'pm_reports_closed':
				...
				$limit_time_sql = ($sort_days) ? 'AND r.report_time >= ' . (cheng_time() - ($sort_days * 86400)) : '';
				...
				{
          // cheng-hack: p=phpbb_privmsgs
          //$sql = 'SELECT p.*,
          $sql = 'SELECT p.msg_id, p.root_level, p.author_id, p.icon_id, p.author_ip, p.message_time , p.enable_bbcode ,
            p.enable_smilies , p.enable_magic_url, p.enable_sig, p.message_subject, p.message_text , p.message_edit_reason ,
            p.message_edit_user , p.message_attachment , p.bbcode_bitfield , p.bbcode_uid , p.message_edit_time, p.message_edit_count,
            p.to_address , p.bcc_address , p.message_reported , 
            u.username, u.username_clean, u.user_colour, r.user_id as reporter_id, ru.username as reporter_name, ru.user_colour as reporter_colour, r.report_time, r.report_id
