	...
	*/
	static public function approve_posts($action, $post_id_list, $id, $mode)
	{
		...
		if (confirm_box(true))
		{
			...
			{
				$phpbb_content_visibility->set_post_visibility(ITEM_APPROVED, $topic_data['posts'], $topic_id, $topic_data['forum_id'], $user->data['user_id'], cheng_time(), '', isset($topic_data['first_post']), isset($topic_data['last_post']));
			}
	...
	*/
	static public function approve_topics($action, $topic_id_list, $id, $mode)
	{
		...
		if (confirm_box(true))
		{
			...
			foreach ($topic_info as $topic_id => $topic_data)
			{
				$phpbb_content_visibility->set_topic_visibility(ITEM_APPROVED, $topic_id, $topic_data['forum_id'], $user->data['user_id'], cheng_time(), '');
