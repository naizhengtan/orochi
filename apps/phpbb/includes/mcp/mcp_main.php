...
function mcp_restore_topic($topic_ids)
{
	...
	if (confirm_box(true))
	{
		...
		foreach ($data as $topic_id => $row)
		{
			$return = $phpbb_content_visibility->set_topic_visibility(ITEM_APPROVED, $topic_id, $row['forum_id'], $user->data['user_id'], cheng_time(), '');
			if (!empty($return))
			{
		...
		foreach ($data as $topic_id => $row)
		{
			...
			else
			{
				...
				if ($is_soft)
				{
					/* @var $phpbb_content_visibility \phpbb\content_visibility */
					$phpbb_content_visibility = $phpbb_container->get('content.visibility');
					$return = $phpbb_content_visibility->set_topic_visibility(ITEM_DELETED, $topic_id, $row['forum_id'], $user->data['user_id'], cheng_time(), $soft_delete_reason);
					if (!empty($return))
					{
...
function mcp_delete_post($post_ids, $is_soft = false, $soft_delete_reason = '', $action = 'delete_post')
{
	...
	if (confirm_box(true) && $is_soft)
	{
		...
		{
			$phpbb_content_visibility->set_post_visibility(ITEM_DELETED, $topic_data['posts'], $topic_id, $topic_data['forum_id'], $user->data['user_id'], cheng_time(), $soft_delete_reason, isset($topic_data['first_post']), isset($topic_data['last_post']));
		}
