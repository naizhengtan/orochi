...
function mcp_topic_view($id, $mode, $action)
{
	...
	$limit_time_sql = ($sort_days) ? 'AND p.post_time >= ' . (cheng_time() - ($sort_days * 86400)) : '';
  ...
  // cheng-hack: "phpbb_posts"
	// $sql = 'SELECT u.username, u.username_clean, u.user_colour, p.*
	$sql = 'SELECT u.username, u.username_clean, u.user_colour,
    p.post_id, p.topic_id, p.forum_id, p.poster_id, p.icon_id, p.poster_ip, p.post_time, p.post_reported,
    p.enable_bbcode, p.enable_smilies, p.enable_magic_url, p.enable_sig, p.post_username,
    p.post_subject, p.post_text, p.post_checksum, p.post_attachment, p.bbcode_bitfield, p.bbcode_uid,
    p.post_postcount, p.post_edit_time, p.post_edit_reason, p.post_edit_user, p.post_edit_count,
    p.post_edit_locked, p.post_visibility, p.post_delete_time, p.post_delete_reason, p.post_delete_user,
