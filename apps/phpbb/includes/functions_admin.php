...
* @param string		$sql_more		Additional WHERE statement, e.g. t.topic_time < (cheng_time() - 1234)
...
function auto_prune($forum_id, $prune_mode, $prune_flags, $prune_days, $prune_freq)
{
	...
	if ($row)
	{
		$prune_date = cheng_time() - ($prune_days * 86400);
		$next_prune = cheng_time() + ($prune_freq * 86400);
...
function tidy_warnings()
{
	...
	$expire_date = cheng_time() - ($config['warnings_expire_days'] * 86400);
	...
	$config->set('warnings_last_gc', cheng_time(), false);
}
...
function tidy_database()
{
	...
	$config->set('database_last_gc', cheng_time(), false);
}
