...
class acp_main
{
	...
	function main($id, $mode)
	{
			...
			else
			{
				switch ($action)
				{
					...
					case 'date':
						...
						$config->set('board_startdate', cheng_time() - 1);
					...
					case 'db_track':
						...
						$get_from_time = cheng_time() - (6 * 4 * 7 * 24 * 60 * 60);
		...
		*/
		...
		$boarddays = (cheng_time() - $config['board_startdate']) / 86400;
		...
		if ($config['allow_attachments'] || $config['allow_pm_attach'])
		{
			$sql = 'SELECT COUNT(attach_id) AS total_orphan
				FROM ' . ATTACHMENTS_TABLE . '
				WHERE is_orphan = 1
					AND filetime < ' . (cheng_time() - 3*60*60);
			$result = $db->sql_query($sql);
