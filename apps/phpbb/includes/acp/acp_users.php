	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			case 'feedback':
				...
				$sql_where = ($sort_days) ? (cheng_time() - ($sort_days * 86400)) : 0;
				...
				*/
				...
				if ($submit)
				{
					$error = validate_data($data, array(
						...
						'bday_year'		=> array('num', true, 1901, cheng_gmdate('Y', cheng_time())),
			...
			case 'prefs':
				...
				*/
				...
				foreach ($user->lang['dateformats'] as $format => $null)
				{
					...
					$dateformat_options .= $user->format_date(cheng_time(), $format, false) . ((strpos($format, '|') !== false) ? $user->lang['VARIANT_DATE_SEPARATOR'] . $user->format_date(cheng_time(), $format, true) : '');
