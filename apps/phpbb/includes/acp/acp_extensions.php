		...
		*/
		...
		switch ($action)
		{
			...
			case 'enable':
				...
				try
				{
					while ($phpbb_extension_manager->enable_step($ext_name))
					{
						...
						if ((cheng_time() - $start_time) >= $safe_time_limit)
						{
					...
					$this->log->add('admin', $user->data['user_id'], $user->ip, 'LOG_EXT_ENABLE', cheng_time(), array($ext_name));
				}
			...
			case 'disable':
				...
				$this->log->add('admin', $user->data['user_id'], $user->ip, 'LOG_EXT_DISABLE', cheng_time(), array($ext_name));
			...
			case 'delete_data':
				...
				try
				{
					...
					$this->log->add('admin', $user->data['user_id'], $user->ip, 'LOG_EXT_PURGE', cheng_time(), array($ext_name));
				}
				catch (\phpbb\db\migration\exception $e)
				{
					$template->assign_var('MIGRATOR_ERROR', $e->getLocalisedMessage($user));
