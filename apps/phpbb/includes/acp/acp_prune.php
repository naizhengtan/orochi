	...
	function prune_forums($id, $mode)
	{
		...
		if ($submit)
		{
			if (confirm_box(true))
			{
				...
				$prunedate_posted = cheng_time() - ($prune_posted * 86400);
				$prunedate_viewed = cheng_time() - ($prune_viewed * 86400);
				...
				if ($row = $db->sql_fetchrow($result))
				{
					...
					do
					{
						...
						if ($prune_all)
						{
							$p_result = prune($row['forum_id'], 'posted', cheng_time(), $prune_flags, false);
						}
						else
						{
							if ($prune_posted)
