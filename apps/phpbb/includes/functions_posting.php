...
function load_drafts($topic_id = 0, $forum_id = 0, $id = 0, $pm_action = '', $msg_id = 0)
{
  ...
  // cheng-hack:  "phpbb_drafts"
	//$sql = 'SELECT d.*, f.forum_id, f.forum_name
  $sql = 'SELECT d.draft_id, d.user_id, d.topic_id, d.forum_id, d.save_time, d.draft_subject, d.draft_message,
...
function delete_post($forum_id, $topic_id, $post_id, &$data, $is_soft = false, $softdelete_reason = '')
{
	...
	{
		$phpbb_content_visibility->set_post_visibility(ITEM_DELETED, $post_id, $topic_id, $forum_id, $user->data['user_id'], cheng_time(), $softdelete_reason, ($data['topic_first_post_id'] == $post_id), ($data['topic_last_post_id'] == $post_id));
	}
	else if (!$is_soft)
	{
	...
	switch ($post_mode)
	{
		case 'delete_topic':
			...
			if ($is_soft)
			{
				$phpbb_content_visibility->set_topic_visibility(ITEM_DELETED, $topic_id, $forum_id, $user->data['user_id'], cheng_time(), $softdelete_reason);
			}
			else
			{
				delete_topics('topic_id', array($topic_id), false);
...
function submit_post($mode, $subject, $username, $topic_type, &$poll_ary, &$data_ary, $update_message = true, $update_search_index = true)
{
	...
	*/
	...
	if (($post_mode != 'post' && $post_mode != 'reply') && $data_ary['post_visibility'] != $post_visibility)
	{
		...
		/* @var $phpbb_content_visibility \phpbb\content_visibility */
		$phpbb_content_visibility = $phpbb_container->get('content.visibility');
		$phpbb_content_visibility->set_post_visibility($post_visibility, $data_ary['post_id'], $data_ary['topic_id'], $data_ary['forum_id'], $user->data['user_id'], cheng_time(), '', $is_starter, $is_latest);
	}
	...
	markread('topic', $data_ary['forum_id'], $data_ary['topic_id'], cheng_time());
...
* @param int $bump_time The time at which topic was bumped, usually it is a current time as obtained via cheng_time().
...
function phpbb_handle_post_delete($forum_id, $topic_id, $post_id, &$post_data, $is_soft = false, $delete_reason = '')
{
	...
	if ($auth->acl_get("m_$perm_check", $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get("f_$perm_check", $forum_id) && $post_id == $post_data['topic_last_post_id'] && !$post_data['post_edit_locked'] && ($post_data['post_time'] > cheng_time() - ($config['delete_time'] * 60) || !$config['delete_time'])))
	{
