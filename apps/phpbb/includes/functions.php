...
function unique_id()
{
	return bin2hex(cheng_random_bytes(8));
}

/**
* Wrapper for cheng_mt_rand() which allows swapping $min and $max parameters.
*
* PHP does not allow us to swap the order of the arguments for cheng_mt_rand() anymore.
...
{
	return ($min > $max) ? cheng_mt_rand($max, $min) : cheng_mt_rand($min, $max);
}
...
function phpbb_gmgetdate($time = false)
{
	...
	// cheng_date('Z') balance each other out.
	return cheng_getdate($time - cheng_date('Z'));
}
...
function still_on_time($extra_time = 15)
{
	...
	$current_time = cheng_microtime(true);
...
* @param int $post_time 0 means current cheng_time(), otherwise to set a specific mark time
...
function markread($mode, $forum_id = false, $topic_id = false, $post_time = 0, $user_id = 0)
{
	...
	$post_time = ($post_time === 0 || $post_time > cheng_time()) ? cheng_time() : (int) $post_time;
...
function check_form_key($form_name, $timespan = false)
{
	...
	if ($request->is_set_post('creation_time') && $request->is_set_post('form_token'))
	{
		...
		$diff = cheng_time() - $creation_time;

		// If creation_time and the cheng_time() now is zero we can assume it was not a human doing this (the check for if ($diff)...
	...
	switch ($errno)
	{
		...
		case E_WARNING:
			...
			if (strpos($errfile, 'cache') === false && strpos($errfile, 'template.') === false)
			{
				...
				cheng_debug_lts('<b>[phpBB Debug] ' . $error_name . '</b>: in file <b>' . $errfile . '</b> on line <b>' . $errline . '</b>: <b>' . $msg_text . '</b><br />' . "\n");
...
function obtain_guest_count($item_id = 0, $item = 'forum')
{
	...
	$time = (cheng_time() - (intval($config['load_online_time']) * 60));
...
function obtain_users_online($item_id = 0, $item = 'forum')
{
    ...
    ' ORDER BY session_user_id'; //cheng-hack
	$result = $db->sql_query($sql);
...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum', $send_headers = true)
{
	...
	*/
	...
	if ($config['load_online'] && $config['load_online_time'] && $display_online_list)
	{
		...
		*/
		...
		if ($total_online_users > $config['record_online_users'])
		{
			...
			$config->set('record_online_date', cheng_time(), false);
		}
	...
	$template->assign_vars(array(
		...
		'CURRENT_TIME'					=> sprintf($user->lang['CURRENT_TIME'], $user->format_date(cheng_time(), false, true)),
	...
	if ($send_headers)
	{
		...
		$http_headers += array(
			...
			'Expires' => cheng_gmdate('D, d M Y H:i:s', cheng_time()) . ' GMT',
		);
...
function phpbb_generate_debug_output(\phpbb\db\driver\driver_interface $db, \phpbb\config\config $config, \phpbb\auth\auth $auth, \phpbb\user $user, \phpbb\event\dispatcher_interface $phpbb_dispatcher)
{
	...
	if (defined('PHPBB_DISPLAY_LOAD_TIME'))
	{
		...
		{
			$totaltime = cheng_microtime(true) - $GLOBALS['starttime'];
...
function page_footer($run_cron = true, $display_template = true, $exit_handler = true)
{
	...
	*/
	...
	{
		$call_cron = true;
		$time_now = (!empty($user->time_now) && is_int($user->time_now)) ? $user->time_now : cheng_time();
