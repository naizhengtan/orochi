...
function view_message($id, $mode, $folder_id, $msg_id, $folder, $message_row)
{
	...
	$msg_data = array(
		...
		'U_EDIT'			=> (($message_row['message_time'] > cheng_time() - ($config['pm_edit_time'] * 60) || !$config['pm_edit_time']) && $folder_id == PRIVMSGS_OUTBOX && $auth->acl_get('u_pm_edit')) ? "$url&amp;mode=compose&amp;action=edit&amp;f=$folder_id&amp;p=" . $message_row['msg_id'] : '',
...
function get_user_information($user_id, $user_row)
{
	...
	if ($config['load_onlinetrack'])
	{
		...
		if ($row)
		{
			$user_row['online'] = (cheng_time() - $update_time < $row['online_time'] && ($row['viewonline'] || $auth->acl_get('u_viewonline'))) ? true : false;
		}
	}
