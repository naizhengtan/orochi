			...
			case 'reg_details':
				...
				*/
					...
					*/
					...
					if (!sizeof($error))
					{
						$sql_ary = array(
							...
							'user_passchg'		=> ($auth->acl_get('u_chgpasswd') && $data['new_password']) ? cheng_time() : 0,
						);
						...
						if ($auth->acl_get('u_chgemail') && $config['email_enable'] && $data['email'] != $user->data['user_email'] && $user->data['user_type'] != USER_FOUNDER && ($config['require_activation'] == USER_ACTIVATION_SELF || $config['require_activation'] == USER_ACTIVATION_ADMIN))
						{
							...
							$user_actkey = gen_rand_string(cheng_mt_rand(6, 10));
							...
							if ($config['require_activation'] == USER_ACTIVATION_ADMIN)
							{
								...
								$notifications_manager->add_notifications('notification.type.admin_activate_user', array(
									...
									'user_regdate'	=> cheng_time(), // Notification time
								));
							}
				...
				$template->assign_vars(array(
					...
					'S_FORCE_PASSWORD'	=> ($auth->acl_get('u_chgpasswd') && $config['chg_passforce'] && $user->data['user_passchg'] < cheng_time() - ($config['chg_passforce'] * 86400)) ? true : false,
				...
				if ($submit)
				{
					...
					if ($config['allow_birthdays'])
					{
						$validate_array = array_merge($validate_array, array(
							...
							'bday_year'		=> array('num', true, 1901, cheng_gmdate('Y', cheng_time()) + 50),
