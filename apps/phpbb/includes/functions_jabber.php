...
/*		if (!$this->check_jid($this->username . '@' . $this->server))
	...
	*/
	function listen($timeout = 10, $wait = false)
	{
		...
		while (cheng_time() <= $start + $timeout && !feof($this->connection) && ($wait || $data == '' || $read != '' || (substr(rtrim($data), -1) != '>')));
	...
	*/
	function response($xml)
	{
		...
		switch (key($xml))
		{
			...
			case 'challenge':
				...
				$decoded['cnonce'] = base64_encode(md5(cheng_uniqid(cheng_mt_rand(), true)));
	...
	function send_message($to, $text, $subject = '', $type = 'normal')
	{
		...
		return $this->send("<message from='" . utf8_htmlspecialchars($this->session['jid']) . "' to='" . utf8_htmlspecialchars($to) . "' type='$type' id='" . cheng_uniqid('msg') . "'>
