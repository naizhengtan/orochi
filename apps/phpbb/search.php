...
{
	if ($user->data['user_last_search'] > cheng_time() - $interval)
	{
		$template->assign_var('S_NO_SEARCH', true);
		trigger_error($user->lang('NO_SEARCH_TIME', (int) ($user->data['user_last_search'] + $interval - cheng_time())));
	}
}
	...
	if ($search_id)
	{
		switch ($search_id)
		{
			...
			case 'active_topics':
				...
				$last_post_time_sql = ($sort_days) ? ' AND t.topic_last_post_time > ' . (cheng_time() - ($sort_days * 24 * 3600)) : '';
			...
			case 'unanswered':
				...
				if ($sort_days)
				{
					$last_post_time = 'AND p.post_time > ' . (cheng_time() - ($sort_days * 24 * 3600));
				}
				else
				{
					$last_post_time = '';
			...
			case 'unreadposts':
				...
				$template->assign_var('U_MARK_ALL_READ', ($user->data['is_registered'] || $config['load_anon_lastread']) ? append_sid("{$phpbb_root_path}index.$phpEx", 'hash=' . generate_link_hash('global') . '&amp;mark=forums&amp;mark_time=' . cheng_time()) : '');
			break;
