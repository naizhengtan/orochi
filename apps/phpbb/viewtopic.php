...
if (($topic_data['topic_type'] == POST_STICKY || $topic_data['topic_type'] == POST_ANNOUNCE) && $topic_data['topic_time_limit'] && ($topic_data['topic_time'] + $topic_data['topic_time_limit']) < cheng_time())
{
...
if ($sort_days)
{
	$min_post_time = cheng_time() - ($sort_days * 86400);
...
{
  // cheng-hack: assume this is table "phpbb_poll_options"
	//$sql = 'SELECT o.*, p.bbcode_bitfield, p.bbcode_uid
	$sql = 'SELECT o.poll_option_id, o.topic_id, o.poll_option_text, o.poll_option_total, p.bbcode_bitfield, p.bbcode_uid
	...
	$s_can_vote = ($auth->acl_get('f_vote', $forum_id) &&
		(($topic_data['poll_length'] != 0 && $topic_data['poll_start'] + $topic_data['poll_length'] > cheng_time()) || $topic_data['poll_length'] == 0) &&
	...
	*/
	...
	if ($update && $s_can_vote)
	{
		...
		{
			$user->set_cookie('poll_' . $topic_id, implode(',', $voted_id), cheng_time() + 31536000);
		}

		$sql = 'UPDATE ' . TOPICS_TABLE . '
			SET poll_last_vote = ' . cheng_time() . "
			WHERE topic_id = $topic_id";
		//, topic_last_post_time = ' . cheng_time() . " -- for bumping topics with new votes, ignore for now
		$db->sql_query($sql);
	...
	$poll_template_data = array(
		...
		'L_POLL_LENGTH'		=> ($topic_data['poll_length']) ? sprintf($user->lang[($poll_end > cheng_time()) ? 'POLL_RUN_TILL' : 'POLL_ENDED_AT'], $user->format_date($poll_end)) : '',
...
if ($config['load_onlinetrack'] && sizeof($id_cache))
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		$user_cache[$row['session_user_id']]['online'] = (cheng_time() - $update_time < $row['online_time'] && (($row['viewonline']) || $auth->acl_get('u_viewonline'))) ? true : false;
	}
	$db->sql_freeresult($result);
}
unset($id_cache);
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$s_cannot_edit_time = $config['edit_time'] && $row['post_time'] <= cheng_time() - ($config['edit_time'] * 60);
	...
	$s_cannot_delete_time = $config['delete_time'] && $row['post_time'] <= cheng_time() - ($config['delete_time'] * 60);
...
{
  // wait 60s to update
  // cheng-hack: check the last update time
  $user_id = $user->data['user_id'];
  $last_update_id = "_LAST_POST_UPDATE_".$user_id;
  $post_count_id = "_POST_COUNT_".$user_id;
  $last_post_time = $cache->get_driver()->_read($last_update_id);
  $post_count = $cache->get_driver()->_read($post_count_id);

  // only update when last update is 60s ago
  if (!$last_post_time || (cheng_microtime(true) - $last_post_time) > 60) {
    if (!is_int($post_count)) $post_count=1;
    $sql = 'UPDATE ' . TOPICS_TABLE . '
      SET topic_views = topic_views + ' . $post_count . ', topic_last_view_time = ' . cheng_time() . "
      WHERE topic_id = $topic_id";
    $db->sql_query($sql);
    ...
    if (sizeof($update_count))
    {
      $sql = 'UPDATE ' . ATTACHMENTS_TABLE . '
        SET download_count = download_count + ' . $post_count . ' 
        WHERE ' . $db->sql_in_set('attach_id', array_unique($update_count));
      $db->sql_query($sql);
    }

    $cache->get_driver()->_write($last_update_id, cheng_microtime(true)); 
    $cache->get_driver()->_write($post_count_id, 1);
  } else {
    if ($post_count === FALSE) { // first time
      $cache->get_driver()->_write($post_count_id, 1);
    } else {
      $cache->get_driver()->_write($post_count_id, $post_count+1);
    }
  }
}
