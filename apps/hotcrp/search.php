...
if (($getaction == "paper" || $getaction == "final"
    ...
    cheng_session_write_close();      // to allow concurrent clicks
        ...
        if ($name == $fdef->name && $expr == $fdef->expression)
            ...
            if (!$formula->check())
                ...
                if ($exprViewScore <= $Me->permissive_view_score_bound())
                    ...
                    $changes[] = "insert into Formula (name, heading, headingTitle, expression, authorView, createdBy, timeModified) values ('" . sqlq($name) . "', '', '', '" . sqlq($expr) . "', $exprViewScore, " . ($Me->privChair ? -$Me->contactId : $Me->contactId) . ", " . cheng_time() . ")";
                    if (!$Conf->setting("formulas"))
                        $changes[] = "insert into Settings (name, value) values ('formulas', 1) on duplicate key update value=1";
                } else
                    $changes[] = "update Formula set name='" . sqlq($name) . "', expression='" . sqlq($expr) . "', authorView=$exprViewScore, timeModified=" . cheng_time() . " where formulaId=$fdef->formulaId";
            }
        }
    }
