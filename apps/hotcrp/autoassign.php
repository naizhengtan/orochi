...
class AutoassignerInterface {
    ...
    private function result_html() {
        ...
        if (@$_REQUEST["profile"] && $this->atype_review) {
            ...
            echo 'mean ', sprintf("%.2f", $usum / count($umap)),
                ...
                '<br/>Time: ', sprintf("%.6f", cheng_microtime(true) - $this->start_at);
    ...
    public function progress($status) {
        global $Conf;
        if ($this->live && cheng_microtime(true) - $this->start_at > 1) {
            $this->live = false;
            echo "</div>\n";
            $Conf->echoScript("");
        }
    ...
    public function run() {
        ...
        assert($this->ok);
        cheng_session_write_close(); // this might take a long time
        set_time_limit(240);
        ...
        $this->start_at = cheng_microtime(true);
