...
class Dbl {
    ...
    static public $check_warnings = false;
    ...
    static function connect_dsn($dsn) {
        ...
        if ($dblink && !mysqli_connect_errno() && $dblink->select_db($dbname)) {
            ...
            oro_mysql_query($dblink, "set group_concat_max_len=4294967295");
        } else if ($dblink) {
            $dblink->close();
            $dblink = null;
        }
    ...
    static private function do_query($args, $flags) {
        ...
        $result = oro_mysql_query($dblink, $qstr);
        if ($result === true)
            $result = $dblink;
        ...
        if (self::$check_warnings && !($flags & self::F_ALLOWERROR)
            ...
            $wresult = oro_mysql_query($dblink, "show warnings");
