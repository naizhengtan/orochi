...
while [ $# -gt 0 ]; do
    ...
    --no-schema)
        no_schema=true;;
    --tt-mode)
        echo "*** Set Time Travel Mode to True ***"
        cheng_tt_mode=true;;
    --no-setup-phase)
...
if ! $no_schema; then
    ...
    # cheng-hack:
    echo $cheng_tt_mode
    if $cheng_tt_mode; then
      # used to be:
      #$qecho "+ $setup_phase ${SRCDIR}schema_new_tt.sql | $MYSQL$myargs_redacted$FLAGS $DBNAME"
      # lingfan's new sql
      $qecho "+ $setup_phase ${SRCDIR}lingfan_schema_tt.sql | $MYSQL$myargs_redacted$FLAGS $DBNAME"
      $qecho "*** This is Time Travel mode ***"
      eval $setup_phase ${SRCDIR}schema_new_tt.sql | eval $MYSQL $myargs $FLAGS $DBNAME || exit 1
    else
      $qecho "+ $setup_phase ${SRCDIR}schema_normal.sql | $MYSQL$myargs_redacted$FLAGS $DBNAME"
      eval $setup_phase ${SRCDIR}schema_normal.sql | eval $MYSQL $myargs $FLAGS $DBNAME || exit 1
    fi
fi
...
# cheng-hack: we want to regenerate options all the time
