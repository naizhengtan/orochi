...
function fileUploaded(&$var) {
    ...
    switch ($var['error']) {
    case UPLOAD_ERR_OK:
        return cheng_is_uploaded_file($var['tmp_name'])
            || (!defined("CHENG") && PHP_SAPI === "cli" && @$var["tmp_name_safe"]);
...
function clean_tempdirs() {
    ...
    while (($fname = readdir($dirh)) !== false)
        if (preg_match('/\Ahotcrptmp\d+\z/', $fname)
            && is_dir("$dir/$fname")
            && ($mtime = @cheng_filemtime("$dir/$fname")) !== false
            && $mtime < $now - 1800) {
...
function tempdir($mode = 0700) {
    ...
    while (substr($dir, -1) == "/")
        $dir = substr($dir, 0, -1);
    for ($i = 0; $i < 100; $i++) {
        $path = $dir . "/hotcrptmp" . cheng_mt_rand(0, 9999999);
        if (mkdir($path, $mode))
            return $path;
    }
    return false;
...
function hotcrp_random_bytes($length = 16, $secure_only = false) {
  /*
    ...
    if (($key === false || $key === "")
        ...
        $key = cheng_openssl_random_pseudo_bytes($length, $strong);
        $key = ($strong ? $key : false);
    }
    if (($key === false || $key === "") && !$secure_only) {
        $key = "";
        while (strlen($key) < $length)
            $key .= pack("V", cheng_mt_rand());
...
function hotcrp_random_password($length = 14) {
    ...
    if ($bytes === false) {
        ...
        while (strlen($bytes) < $length)
            $bytes .= sha1($Opt["conferenceKey"] . pack("V", cheng_mt_rand()));
    }
