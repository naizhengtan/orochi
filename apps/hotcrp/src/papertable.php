...
class PaperTable {
    ...
    private function _analyze_authors() {
        ...
        // FIXME: This will be ambiguous for TTdb, change a little bit
        ...
        $result = $Conf->qe("select firstName, lastName, email, '' as affiliation, ContactInfo.contactId
                from ContactInfo join PaperConflict using (contactId)
                where paperId=" . $this->prow->paperId . " and conflictType>=" . CONFLICT_AUTHOR);
        $contacts = array();
    ...
    function deadlineSettingIs($dname) {
        ...
        if ($deadline === "N/A")
            return "";
        else if (cheng_time() < $Conf->setting($dname))
