...
function update_schema_create_review_form($Conf) {
    ...
    // cheng: cannot find table ReviewFormField???
    ...
    // cheng: cannot find table ReviewFormOptions???
...
function update_schema_create_options($Conf) {
    global $reviewScoreNames;
    // cheng: cannot find table OptionType???
...
function update_schema_transfer_address($Conf) {
    // cheng: cannot find table ContactAddress???
  ...
  UNIQUE KEY `capabilityValue` (`capabilityValue`)
    ...
    if ($Conf->settings["allowPaperOption"] == 98) {
        ...
        do {
            ...
            $result = $Conf->ql("select reviewId, paperId, contactId  , reviewToken, reviewType , reviewRound, requestedBy, timeRequested  , timeRequestNotified, reviewBlind, reviewModified , reviewSubmitted, reviewNotified , reviewAuthorNotified , reviewAuthorSeen , reviewOrdinal  , reviewEditVersion, reviewNeedsSubmit, overAllMerit , reviewerQualification, novelty, technicalMerit , interestToCommunity, longevity  , grammar, likelyPresentation , suitableForShort , paperSummary , commentsToAuthor , commentsToPC , commentsToAddress, weaknessOfPaper, strengthOfPaper, potential  , fixability , textField7 , textField8 , reviewWordCount  from PaperReview where reviewWordCount is null limit 32");
