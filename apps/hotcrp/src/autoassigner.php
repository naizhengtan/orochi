...
class Autoassigner {
    ...
    private function preferences_review() {
        ...
        $this->profile["preferences"] = cheng_microtime(true) - $time;
    }
    ...
    private function assign_stupidly(&$papers, $action, $round, $nperpc) {
        ...
        while (count($pcm)) {
            ...
            while (count($apids)) {
                $pididx = cheng_mt_rand(0, count($apids) - 1);
                $pid = $apids[$pididx];
    ...
    private function assign_randomly(&$papers, $action, $round, $nperpc) {
        ...
        while (count($pcm)) {
            ...
            while ($this->pref_groups[$pc]
                ...
                $pididx = cheng_mt_rand(0, count($pg->apids) - 1);
                $pid = $pg->apids[$pididx];
    ...
    private function assign_mcmf_once(&$papers, $action, $round, $nperpc) {
        ...
        $this->profile["traverse"] = cheng_microtime(true) - $time;
        return $nassigned;
    }
    ...
    private function run_discussion_order_once($conf, $plist) {
        ...
        while (count($roots)) {
            ...
            if (count($roots) !== count($plist))
                $source = "p" . $roots[cheng_mt_rand(0, count($roots) - 1)];
            $pgroup = $igroup = array();
