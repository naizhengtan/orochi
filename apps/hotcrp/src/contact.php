...
class Contact {
    ...
    static public function contactdb_find_by_email($email) {
        ...
        if (($cdb = self::contactdb())) {
            $result = Dbl::ql($cdb, "select contactId, firstName, lastName, unaccentedName, email, preferredEmail, affiliation, voicePhoneNumber, faxPhoneNumber, password, passwordTime, passwordIsCdb, collaborators, creationTime, lastLogin, defaultWatch, roles, disabled, contactTags, data from ContactInfo where email=?", $email);
    ...
    static public function contactdb_find_by_id($cid) {
        ...
        if (($cdb = self::contactdb())) {
            ...
            $result = Dbl::ql($cdb, "select contactId, firstName, lastName, unaccentedName, email, preferredEmail, affiliation, voicePhoneNumber, faxPhoneNumber, password, passwordTime, passwordIsCdb, collaborators, creationTime, lastLogin, defaultWatch, roles, disabled, contactTags, data from ContactInfo where contactDbId=?", $cid);
            $acct = $result ? $result->fetch_object("Contact") : null;
            Dbl::free($result);
        }
        return $acct;
    ...
    function save_json($cj, $actor, $send) {
        ...
        if (!($result = Dbl::qe_apply($Conf->dblink, $q, $qv)))
            return $result;
        if ($inserting) {
          $this->contactId = $this->cid = (int) cheng_mysqli_insert_id($result);
        }
        Dbl::free($result);
    ...
    private function load_by_id($cid) {
        $result = Dbl::q("select contactId, firstName, lastName, unaccentedName, email, preferredEmail, affiliation, voicePhoneNumber, faxPhoneNumber, password, passwordTime, passwordIsCdb, collaborators, creationTime, lastLogin, defaultWatch, roles, disabled, contactTags, data from ContactInfo where contactId=?", $cid);
    ...
    static function find_by_id($cid) {
        $result = Dbl::qe("select contactId, firstName, lastName, unaccentedName, email, preferredEmail, affiliation, voicePhoneNumber, faxPhoneNumber, password, passwordTime, passwordIsCdb, collaborators, creationTime, lastLogin, defaultWatch, roles, disabled, contactTags, data from ContactInfo where contactId=?", $cid);
    ...
    static function find_by_email($email) {
        ...
        if (($email = trim((string) $email)) !== "") {
            ...
            $result = Dbl::qe("select contactId, firstName, lastName, unaccentedName, email, preferredEmail, affiliation, voicePhoneNumber, faxPhoneNumber, password, passwordTime, passwordIsCdb, collaborators, creationTime, lastLogin, defaultWatch, roles, disabled, contactTags, data from ContactInfo where email=?", $email);
            $acct = $result ? $result->fetch_object("Contact") : null;
            Dbl::free($result);
        }
        return $acct;
    ...
    function sendAccountInfo($sendtype, $sensitive) {
        ...
        if ($sendtype == "create" && $this->prefer_contactdb_password())
            ...
            $rest["capability"] = $capmgr->create(CAPTYPE_RESETPASSWORD, array("user" => $this, "timeExpires" => cheng_time() + 259200));
    ...
    private static function unassigned_review_token() {
        while (1) {
            $token = cheng_mt_rand(1, 2000000000);
    ...
    function assign_review($pid, $reviewer_cid, $type, $extra = array()) {
        ...
        if ($q[0] == "d") {
            ...
            $reviewId = cheng_mysqli_insert_id($result);
        }
