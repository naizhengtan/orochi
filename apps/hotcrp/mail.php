...
if (isset($_REQUEST["fromlog"]) && ctype_digit($_REQUEST["fromlog"])
    ...
    $result = $Conf->qe("select mailId, recipients, q, t, paperIds, cc, replyto, subject, emailBody from MailLog where mailId=" . $_REQUEST["fromlog"]);
    ...
    private function run() {
        ...
        if ($this->sending) {
            ...
            if (($log_result = Dbl::query_raw("insert into MailLog set $q")))
                $this->mailid_text = " #" . cheng_mysqli_insert_id($log_result);
        ...
        if (!$this->started && !count($preperrors))
            ...
            $this->echo_actions();
        if ($revinform)
            $Conf->qe("update PaperReview set timeRequestNotified=" . cheng_time() . " where " . join(" or ", $revinform));
        echo "</div></form>";
...
if ($Me->privChair) {
  ...
  $result = $Conf->qe("select mailId, recipients, q, t, paperIds, cc, replyto, subject, emailBody from MailLog order by mailId desc limit 18");
    if (edb_nrows($result)) {
